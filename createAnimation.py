...
def convert_images():
    ...
    for png_file in png_list:
        ...
        resize_image(png_path)
        png_path = png_path.replace("/imagesIn/", "/imagesResized/")
    animation_path = combine_frames()
    #reduce_png_pallete(animation_path) 
...
def combine_frames():
    ...
    png_list = glob.glob(path)
    #Getting frame size
    tempImg = Image.open(png_list[0]).convert('RGBA')
    frame_size = (tempImg.width, tempImg.height)
    tempImg.close()
    #Getting spritesheet size
    
    spritesheet_size = (frame_size[0]*len(png_list), frame_size[1])
    #Creating spritesheet
    imgSpritesheet = Image.new(mode="RGBA", size=spritesheet_size)
    cur_x = 0
    cur_y = 0
    
    for cur_frame_ind in range(len(png_list)):
        png_file = f"./imagesResized/{cur_frame_ind+1}.png"
        
        tempImg = Image.open(png_file).convert('RGBA')
        imgSpritesheet.paste(tempImg, (cur_x, cur_y))
        tempImg.close()
        
        cur_x += frame_size[0]
    
    save_path = f"./imagesOut/animation-{int(frame_size[0]/8)}_{int(frame_size[1]/8)}_5.png"
...
def resize_image(image_url):
    ...
    output_png_file_path = image_url.replace("/imagesIn/", "/imagesResized/")
